...
backend/audio/interruption.py
It is responsible for tracking which request is currently authoritative.
...
Open the displayed address in your browser and allow microphone access when requested.
...
However, mock mode does not replace the real Rime integration for the hackathon demonstration.
The intended voice demonstration uses the real Rime TTS path.
...
Ask a study question using the microphone.
Verify that the audio is transcribed.
Verify that the LLM generates an answer.
Verify that Rime generates speech.
...
Verify that the current assistant audio stops.
Verify that the new request is processed.
...
Verify that the old response does not resume speaking.
...
This is important because simply stopping browser playback is not enough.
...
Step 6
The user interrupts:
"Actually, explain cellular respiration."
...
Voice Engineering
 ...
 Stale-response behavior documented
...
**One important thing:** before you submit, verify the **Language** row in the Rime table against your actual `.env`/code. Your project currently uses `RIME_LANG` as the configurable language, so I deliberately did **not** falsely hard-code `en` there unless your actual configuration confirms it.
